...
export async function POST(req: NextRequest) {
  try {
    ...
    // 5. Wrap with Universal AI Governance Envelope and enforce NEEDS_REVIEW status
    const envelope = AiGovernance.createSuggestion(
      draft,
      'AI_AUTOFILL',
      0.92,
      ['Generated by product-autofill-engine with verified taxonomy context']
    );

    (draft as any).suggestedState = 'NEEDS_REVIEW';
    (draft as any).aiGenerated = true;
